...
public class TankServer extends Frame {
    ...
    public static final int TCP_PORT = 60000;//TCP端口号
    public static final int UDP_PORT = 60001;//转发客户端数据的UDP端口号
    public static final int TANK_DEAD_UDP_PORT = 60002;//接收客户端坦克死亡的端口号
    ...
    private class UDPThread implements Runnable{//这个线程是接受客户端连接进来的线程，将IP、UDP端口记录到<客户>数组里。
        ...
        public void run() {
            ...
            while(null != ds){
                ...
                try{
                    ...
                    int deadTankUDPPort = dis.readInt();//这里是服务器端一直在监听发来的消息，在客户端有对应的协议，客户端的协议会发送一个UDP端口号，说我已经寄了。
                    for(int i = 0; i < clients.size(); i++){
                        Client c = clients.get(i);
                        if(c.UDP_PORT == deadTankUDPPort){
                            clients.remove(c);//遍历客户端数组，一样就把客户端移除。
                        }
                    }
    ...
    public class Client{//记录客户端信息的类，记下来UDP端口、IP地址和发配的坦克ID。
        String IP;
        int UDP_PORT;
        int id;
