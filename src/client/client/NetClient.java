...
public class NetClient {
    ...
    public void connect(String ip){
        ...
        try {
            ...
            DataInputStream dis = new DataInputStream(s.getInputStream());//只有开始的时候，要发送自己的UDP端口时才需要用TCP和那个socket。
    ...
    public void send(Msg msg){//多态实现，这里的参数事Msg类型，但我们都懂其实里面不一定事Msg类型的消息，也可以是里面的细分，都能发出去。
    ...
    public class UDPThread implements Runnable{
        ...
        @Override
        public void run() {//润方法包括parse的调用，客户端根据收到消息的int部分调用对应方法。
            while(null != ds){
        ...
        private void parse(DatagramPacket dp) {
            ...
            switch (msgType){//根据消息的类型调用对应消息的解析方法，坦克那里的多态用的很妙，每个消息类型都实现了如何解析。
                ...
                case Msg.MESSAGE_MSG:
                	msg = new MessageMsg(tc);
                	msg.parse(dis);
            }
        }
    }

    public void sendClientDisconnectMsg(){//自己关客户端或者坦克死亡时调用。
